...
# Post first tweet :
#client.update('Mon premier tweet')

#Send tweet to 5 journalist au hasard :
...
#journalist_sample = journalist.sample(5)
#journalist_sample.each {|n| client.update("#{n} Salut tout le monde #bonjour_monde")}

#like 25 recent #bonjour_monde :
#like_hello = client.search("#bonjour_monde", result_type: "recent").take(25)
#client.favorite(like_hello)

#Follow les derniers 20 users who have posted #bonjour_monde:
#client.search("#bonjour_monde", result_type: "recent").take(10).each do |post|
#client.follow(post.user)
#end
